...
do
    ...
    # search query
    res=$(gh pr list -s open --search " -author:@me -author:knative-automation -label:size/L -label:size/XL -label:size/XXL" --json author,title,author,createdAt,updatedAt,mergedAt,number,url,state,labels,files)
